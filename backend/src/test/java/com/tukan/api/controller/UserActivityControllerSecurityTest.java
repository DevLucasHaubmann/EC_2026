...
/**
 * Verifies HTTP-layer RBAC for POST /me/activity.
...
class UserActivityControllerSecurityTest {
    ...
    @BeforeEach
    void stubService() {
        ActivityStreakResponse stub = new ActivityStreakResponse(1, 1, 1, LocalDate.now());
        when(userActivityService.recordAccessAndGetStreak(anyString())).thenReturn(stub);
    }
    ...
    @Nested
    @DisplayName("POST /me/activity — USER ativo")
    class ActiveUser {

        @Test
        @DisplayName("USER ativo registra acesso e recebe 200")
        void userCanRecordAccess() throws Exception {
            mockMvc.perform(post("/me/activity")
    ...
    @Nested
    @DisplayName("POST /me/activity — ADMIN")
    ...
    @Nested
    @DisplayName("POST /me/activity — sem token")
